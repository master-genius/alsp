...
文件会自动去掉可执行权限。\n\
    ...
    -r     : 递归设置目录\n\
    [MODE] : 三位数表示8进制的权限码\n\
    -I     : 区分大小写的正则匹配，需要配合--regex使用。\n\
    --regex [REGEX] : 正则表达式搜索，只有匹配成功才会进行权限设置\n\
        正则表达式规则：\n\
          ^  行首匹配\n\
          .  匹配任意字符\n\
          *  前一个字符出现任意次数\n\
...
example:\n\
    setmod -r --auto 755 m/\n\
    setmod -r 644 m/\n\
    setmod -r --regex '\\.py$' 755 www\n\
        所有.py结尾的文件设置为755权限\n\
    setmod -r --regex '\\.php|\\.py' 754 www\n\
        所有.py或是.php结尾的文件设置为754权限\n\
"
...
int recur_dir(struct path_list*ph, int mode, int deep) {
    ...
    int (*mre)(char *, char *, int);

    mre = match;
    if (__regex && strchr(__regex, '|')) {
        mre = matchs;
    }
    int uplow = __args[ARGS_UPLOW];
    ...
    while (pl) {
        ...
        while((rd=readdir(d))!=NULL) {
            ...
            if (__args[ARGS_REGEX]) {
                if (!mre(__regex, rd->d_name, uplow)) {
                    continue;
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    for (int i=1; i<argc; i++) {
        if (strcmp(argv[i], "--regex") == 0) {
            if (__regex) {
                dprintf(2, "Error: too many regex\n");
                return -1;
            }
            i++;
            if (i>=argc) {
                dprintf(2, "Error: less regex\n");
                return -1;
            }
            __args[ARGS_REGEX] = 1;
            __regex = argv[i];
            ...
            __args[ARGS_UPLOW] = 1;
        } else if (strncmp(argv[i], "--auto=", 7) == 0) {
            __args[ARGS_AUTO] = 1;
